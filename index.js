...
app.post('/register', async (req, res) => {
  ...
  try {
    // Проверяем, сколько уже зарегистрировано пользователей
...
app.post('/admin/ban', async (req, res) => {
  ...
  try {
    ...
    await supabase
      ...
      .update({ 
        is_banned: true, 
        ban_reason: reason || 'Причина не указана' 
      })
      .eq('id', user.id);
...
// 6. Получить список всех пользователей (для админа)
